...
def evaluate_marker(text, extra=None, _ops={}):
    ...
    if not _ops:
        ...
        def and_test(nodelist):
            # MUST NOT short-circuit evaluation, or invalid syntax can be
            # skipped!
        ...
        def comparison(nodelist):
            ...
            try:
                ...
                raise SyntaxError(
                    repr(cop) + " operator not allowed in environment markers")
    ...
    def interpret(nodelist):
        ...
        try:
            ...
            raise SyntaxError(
                "Language feature not supported in environment markers: " + symbol.sym_name[nodelist[0]])
        return op(nodelist)
...
class DistInfoDistribution(Distribution):
    ...
    def _parsed_pkg_info(self):
        ...
        try:
            ...
            self._pkg_info = Parser().parsestr(
                self.get_metadata(self.PKG_INFO))
            return self._pkg_info
...
def parse_requirements(strs):
    ...
    def scan_list(ITEM, TERMINATOR, line, p, groups, item_name):
        ...
        while not TERMINATOR(line, p):
            ...
            if not match:
                raise ValueError(
                    "Expected " + item_name + " in", line, "at", line[p:])
    ...
    for line in lines:
        ...
        line, p, specs = scan_list(
            VERSION, LINE_END, line, p, (1, 2), "version spec")
...
class Requirement:
    ...
    def __init__(self, project_name, specs, extras):
        ...
        index = [(parse_version(v), state_machine[op], op, v)
                 for op, v in specs]
        index.sort()
        self.specs = [(op, ver) for parsed, trans, op, ver in index]
        self.index, self.extras = index, tuple(map(safe_extra, extras))
        self.hashCmp = (
            self.key, tuple([(op, parsed)
    ...
    def __contains__(self, item):
        if isinstance(item, Distribution):
            if item.key != self.key:
                return False
            if self.index:
                item = item.parsed_version  # only get if we need it
        ...
        for parsed, trans, op, ver in self.index:
            ...
            if action == 'F':
                ...
                last = False
        if last is None:
            last = True    # no rules encountered
        return last
...
working_set.entries = []
list(
    map(working_set.add_entry, sys.path))  # match order
