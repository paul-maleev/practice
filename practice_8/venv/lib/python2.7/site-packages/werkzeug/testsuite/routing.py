...
class RoutingTestCase(WerkzeugTestCase):
    ...
    def test_path(self):
        ...
        assert adapter.match('/User:thomas') == (
    ...
    def test_alias_redirects(self):
        m = r.Map([
            ...
            r.Rule('/users/index.html', defaults={'page': 1}, alias=True,
                   endpoint='users'),
            r.Rule('/users/page/<int:page>', endpoint='users'),
            r.Rule('/users/page-<int:page>.html',
    ...
    def test_double_defaults(self):
        for prefix in '', '/aaa':
            m = r.Map([
                r.Rule(prefix + '/', defaults={
                       'foo': 1, 'bar': False}, endpoint='x'),
                r.Rule(prefix + '/<int:foo>', defaults={
                       'bar': False}, endpoint='x'),
                r.Rule(prefix + '/bar/', defaults={
                       'foo': 1, 'bar': True}, endpoint='x'),
                r.Rule(prefix + '/bar/<int:foo>',
                       defaults={'bar': True}, endpoint='x')
            ])
            a = m.bind('example.com')
    ...
    def test_host_matching(self):
        m = r.Map([
            ...
            r.Rule(
                '/foo/', defaults={'page': 1}, host='www.<domain>', endpoint='x'),
