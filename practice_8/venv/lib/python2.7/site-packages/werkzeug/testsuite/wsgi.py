...
class WSGIUtilsTestCase(WerkzeugTestCase):
    ...
    def test_path_info_and_script_name_fetching(self):
        ...
        self.assert_equal(
            wsgi.get_path_info(env, charset=None), u'/\N{SNOWMAN}'.encode('utf-8'))
        self.assert_equal(wsgi.get_script_name(env), u'/\N{COMET}')
        self.assert_equal(wsgi.get_script_name(
            env, charset=None), u'/\N{COMET}'.encode('utf-8'))
    ...
    def test_lines_longer_buffer_size(self):
        ...
        for bufsize in range(1, 15):
            lines = list(
                wsgi.make_line_iter(NativeStringIO(data), limit=len(data),
                                    buffer_size=4))
