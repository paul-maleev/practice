...
"""
    ...
    def install_eggs(self, spec, dist_filename, tmpdir):
        ...
        if not os.path.exists(setup_script):
            ...
            if not setups:
                raise DistutilsError(
                    "Couldn't find a setup script in %s" % os.path.abspath(
                        dist_filename)
                )
            if len(setups) > 1:
                raise DistutilsError(
    ...
    def install_exe(self, dist_filename, tmpdir):
        ...
        egg_path = dist.location = os.path.join(
            tmpdir, dist.egg_name() + '.egg')
    ...
    def check_conflicts(self, dist):
        ...
        blockers = []
        names = dict.fromkeys(
            dist._get_metadata('top_level.txt'))  # XXX private attr
...
""" % locals()
    ...
    def unpack_and_compile(self, egg_path, destination):
        ...
        if not self.dry_run:
            for f in to_chmod:
                mode = (
                    (os.stat(f)[stat.ST_MODE]) | 0x16D) & 0xFED  # 0555, 07755
                chmod(f, mode)
        ...
        for name, ep in dist.get_entry_map(group).items():
            ...
            if sys.platform == 'win32' or wininst:
                ...
                if group == 'gui_scripts':
                    ...
                    old = ['.pyw']
                    new_header = re.sub(
                        '(?i)python.exe', 'pythonw.exe', header)
                else:
                    launcher_type = 'cli'
                    ext = '-script.py'
                    old = ['.py', '.pyc', '.pyo']
                    new_header = re.sub(
                        '(?i)pythonw.exe', 'python.exe', header)
